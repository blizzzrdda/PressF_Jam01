...
{
	return Node && !Node->IsPendingKillOrUnreachable();
}
...
{
	if (!GraphHandler.IsValid() || !CommentHandler)
	{
...
bool FBAUtils::HasMetaDataChanged(const FBAVariableDescription& OldVariable, const FBAVariableDescription& NewVariable, const FName Key)
{
	...
	{
		return !OldVariable.GetMetaData(Key).IsEqual(NewVariable.GetMetaData(Key), ENameCase::CaseSensitive);
	}
...
IAssetEditorInstance* FBAUtils::GetEditorFromActiveTab()
{
	...
	{
		if (UBARootObject* RootObject = IBlueprintAssistModule::Get().GetRootObject())
		{
